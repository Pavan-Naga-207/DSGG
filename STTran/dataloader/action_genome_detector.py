...
class AGDetectorStage1(Dataset):
    """
    Frame-level Action Genome dataset for detector pretraining.
    Uses ViT-native RGB inputs and supports square-crop scale jitter.
    """
    ...
    def __init__(
        ...
        self.target_size = int(self.sequence_dataset.vit_input_size)
        self.patch_size = int(self.sequence_dataset.vit_patch_size)
        self.pad_rgb = np.asarray(self.sequence_dataset.vit_pad_rgb, dtype=np.float32)
        self.interpolation = self.sequence_dataset.vit_interpolation
        self.max_crop_retries = max(1, int(os.environ.get('VIT_LSJ_MAX_RETRIES', '10')))
        if mode == 'train':
            self.scale_min = float(os.environ.get('VIT_LSJ_MIN_SCALE', '1.0'))
            self.scale_max = float(os.environ.get('VIT_LSJ_MAX_SCALE', '1.0'))
        else:
            self.scale_min = 1.0
            self.scale_max = 1.0
        ...
        for frame_names, gt_video in zip(
            self.sequence_dataset.video_list,
            self.sequence_dataset.gt_annotations,
        ):
        ...
        print(
            'AGDetectorStage1: {} frames, target_size={}, scale_jitter=[{}, {}]'.format(
                len(self.samples),
                self.target_size,
                self.scale_min,
                self.scale_max,
        ...
        print(
            'AGDetectorStage1 class stats: fg_with_gt={} top_frame_freq={}'.format(
                int(np.count_nonzero(self.class_frame_hist[1:])),
                ', '.join(
                    '{}={}'.format(self.object_classes[idx], int(self.class_frame_hist[idx]))
                    for idx in np.argsort(self.class_frame_hist[1:])[::-1][:5] + 1
                    if self.class_frame_hist[idx] > 0
                ),
            )
        )
    ...
    def _transform_vitdet(self, image, boxes):
        jitter_scale = 1.0
        ...
        im_info = torch.tensor(
            [float(crop_h), float(crop_w), float(resize_scale)],
            dtype=torch.float32,
        )
    ...
    def _transform_keep_all(self, image, boxes):
        ...
        if resized_h < self.target_size or resized_w < self.target_size:
            resized_image = _pad_bottom_right(
                resized_image,
                self.target_size,
                self.target_size,
                self.pad_rgb,
        ...
        im_info = torch.tensor(
            [float(resized_h), float(resized_w), float(resize_scale)],
            dtype=torch.float32,
        )
    ...
    def __getitem__(self, index):
        if self.backbone != 'vitdet':
            raise ValueError('AGDetectorStage1 currently supports only vitdet, got {}'.format(self.backbone))
        ...
        for _sample_attempt in range(3):
            ...
            for _crop_attempt in range(retry_budget):
                candidate_image, candidate_boxes, candidate_im_info = self._transform_vitdet(image, boxes)
        ...
        raise RuntimeError('Failed to build a valid detector sample after retries: {}'.format(last_reason))
